...
def app():
    ...
    df = {}
    for year in years:
        df[year] = pd.read_csv('data/Units by country/Units by country {}.csv'.format(year), thousands=',')
    
    data = df[yearSelected].sort_values(unitTypeSelected, ascending=False)
    fig = go.Figure()
    fig.add_trace(go.Bar(x=data[unitTypeSelected][0:15], y=data['Country'][0:15], orientation='h'))
    fig.update_layout(
    ...
    if yearSelected != years[-1]:
        countries = df[yearSelected]['Country']
        ...
        for country in countries:
            growth = 0
            selectedNumOfUnits = int(df[yearSelected].loc[df[yearSelected]['Country'] == country, unitTypeSelected])
            if (dfPrev['Country'] == country).any():
                previousNumOfUnits = int(dfPrev.loc[dfPrev['Country'] == country, unitTypeSelected])
                if previousNumOfUnits > 0:
                    growth = (selectedNumOfUnits - previousNumOfUnits)
                    growthPerc = growth / previousNumOfUnits * 100
                    df[yearSelected].loc[df[yearSelected]['Country'] == country, 'Unit Growth'] = growth
                    df[yearSelected].loc[df[yearSelected]['Country'] == country, 'Unit Growth Percentage'] = growthPerc
        
        col1, col2 = st.columns(2)

        data = df[yearSelected].sort_values('Unit Growth', ascending=False)
        fig = go.Figure()
        fig.add_trace(go.Bar(x=data['Unit Growth'][0:15], y=data['Country'][0:15], orientation='h'))
        fig.update_layout(
            margin=dict(l=0, r=0, t=40, b=20),
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=data['Unit Growth Percentage'][0:15], y=data['Country'][0:15], orientation='h'))
        fig.update_layout(
            margin=dict(l=0, r=0, t=40, b=20),
            xaxis_title=unitTypeSelected + ' Growth %',
            yaxis={'autorange': 'reversed'}
